...
(function () {
  ...
  /* ── Wallet resolver (works across terraoracle.io + draw.terraoracle.io) ──
     Priority:
       1) window.ORACLE_EYE_WALLET  — set this on a page to point at its own var,
          e.g.  window.ORACLE_EYE_WALLET = function () { return myWallet; };
          or     window.ORACLE_EYE_WALLET = '0x...';
       2) common global variable names used across the Oracle sites
       3) 'anonymous'                                                          */
  function getWallet() {
    try {
      var hook = window.ORACLE_EYE_WALLET;
      if (typeof hook === 'function') { var v = hook(); if (v) return String(v); }
      else if (typeof hook === 'string' && hook) { return hook; }
    } catch (e) {}
    // Known globals across the Oracle sites (typeof works for let/const/var)
    try { if (typeof connectedWalletAddress !== 'undefined' && connectedWalletAddress) return connectedWalletAddress; } catch (e) {}
    try { if (typeof connectedAddress !== 'undefined' && connectedAddress) return connectedAddress; } catch (e) {}
    try { if (typeof globalWalletAddress !== 'undefined' && globalWalletAddress) return globalWalletAddress; } catch (e) {}
    try { if (typeof lotteryAddress !== 'undefined' && lotteryAddress) return lotteryAddress; } catch (e) {}
    // Extra fallbacks via window (covers var-declared globals on other pages)
    var names = ['walletAddress', 'userWallet', 'currentWallet', 'account', 'address'];
    for (var i = 0; i < names.length; i++) {
      try { var w = window[names[i]]; if (typeof w === 'string' && w) return w; } catch (e) {}
    }
    return 'anonymous';
  }
